<template>
  <section>
    <div class="row header-section align-items-center">
      ...
      <div class="col-auto">
        ...
        <AddStuffHistoryComponent
          @get-list="GetAllStuffHistories()"
          ref="addStuffHistoryModal"
        >
        </AddStuffHistoryComponent>
      </div>
    </div>
      ...
      <table class="table table-custom">
        <thead>
          <tr>
            <th
              width="22%"
              @click="OrderBy('stuffName')"
              class="cursor-pointer"
            >
              <font-awesome-icon
                v-if="filter.OrderBy === 'stuffName'"
              ...
              <font-awesome-icon
                v-else-if="filter.OrderBy === 'stuffName_desc'"
        ...
        <tbody>
          <tr
            v-for="(stuffHistory, index) in stuffHistories.Items"
            :key="index"
          >
            <td>
              <div class="d-flex align-items-center">
                <div class="img-container-avatar me-3">
                  <img
                    :src="ShowDynamicImage(stuffHistory.StuffImageBase64)"
                    class="me-2 icon-avatar"
                  />
                </div>
                <span>{{
                  stuffHistory.StuffLastName + " " + stuffHistory.StuffFirstName
                }}</span>
              </div>
            </td>
            ...
            <td>
              <div class="editButtons">
                ...
                <button
                  type="button"
                  class="button-delete"
                  @click="DeleteStuffHistory(stuffHistory.Id)"
                >
    ...
    <Pagination
      :totalPages="stuffHistories.NumberOfPages"
      :currentPage="filter.PageNumber"
      @pagechanged="GetAllStuffHistories"
    />

    <EditStuffHistoryComponent
      :editedStaffHistory="selectedStuffHistoryForEdit"
      @get-list="GetAllStuffHistories"
    />
  </section>
</template>
