    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title mt-2">Adaugă trofeul jucatorului</h5>
          <button
            type="button"
            class="btn-close"
          ...
          <div class="modal-body new-form">
            <div class="mb-2 position-relative">
              <label for="participation" class="form-label"
                >Alege participare</label
              >
              <Field
                v-model="selectedParticipation.PlayerHistoryName"
                name="participation"
                as="select"
                @change="updateSelectedParticipationId"
                :class="{ 'border-danger': errors.participation }"
                class="form-select form-control"
              >
                <option value="" disabled>Participari</option>
                <option
                  v-for="(part, index) in participationsList"
                  :key="index"
                  :value="part.Id"
                >
            ...
            <div class="mb-2 position-relative">
              ...
              <Field
                v-model="selectedTrofee.Name"
                name="player"
                as="select"
                @change="updateSelectedTrofeeId"
                :class="{ 'border-danger': errors.trofee }"
                class="form-select form-control"
              >
                <option value="" disabled>Trofee</option>
                <option
                  v-for="(trofee, index) in trofeesList"
                  :key="index"
                  :value="trofee.Id"
                >
                  {{ trofee.Name }}
...
export default {
  ...
  methods: {
    AddPlayerTrofee() {
      this.$axios
        .post(
          `/api/PlayerTrofee/createPlayerTrofee/${this.selectedParticipation.Id}/${this.selectedTrofee.Id}`
        )
        .then((response) => {
